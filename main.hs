...
tower_heights = [[3, 0, 1, 0], [0, 3, 0, 0], [0, 0, 0, 0], [0, 4, 0, 0]]
...
sprawdz_czy_spelnia_warunek :: (Integer, [Integer]) -> Bool 
sprawdz_czy_spelnia_warunek (el, lista) | el == 1 = fromIntegral(head(lista)) == n
                                        | fromIntegral(el) == n = isSorted lista
                                        | otherwise = sprawdz_roznice_wysokosci lista
                           where n = length lista

-- funkcja realizuja sprawdzenie roznic wysokosci odpowiednich piramid
sprawdz_roznice_wysokosci :: [Integer] -> Bool
sprawdz_roznice_wysokosci lista = length(filter(\n -> n > 0) final_arr) == n
       where n = length lista
             initial_arr = [lista !! 0]
